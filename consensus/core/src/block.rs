...
/// 
/// お疲れ様でした。
/// お弟の結婚式のため時間がなかったのですが、ここまで具体的に教えてくれて本当にありがとうございます。
/// ・ 上記の７つの内容はYeSongママに伝えておきましたか？　　←　伝え済み
/// ・ HSM側に６個の荷物を受ったのかご確認お願い致します。
/// HSMの荷物の中にお兄さんのために靴を2個入れましたが、お兄さんに会って伝えてほしいとHSMに教えてください。
///   ＞＞　対応中です。
/// ・ お兄さんの状態をちょっと確認してみましたか？自分ですべてを判断して決心する状態にはなりますか？
/// 体重はどれくらい見えますか？
///   ＞＞　YeSongパパに確認しましたが、自分が毎週お兄さんの家に行って治療や体調を確認しているそうです。
///   ＞＞　たまに以前のことを忘れる場合があります。
///   ＞＞　かなり回復されていて正常生活ができるほどになりましたが、少し懸念になる点があるぐらいだそうです。
///   ＞＞　YeSongパパが治療を主観していて、お兄さんの妻が世話を良く見てくれていますので、心配しなくて良いそうです。
///   ＞＞　体重は測定したことがありませんが、60kg以上にはなるそうです。退院する時点より太って弱くには見えないそうです。
/// ・ KCJさんは元気にしていますか?私の挨拶を伝えて、必要なものがあれば教えてほしいと言ってください。
///   ＞＞　挨拶を伝えたら誠にありがたいと言われました。
///   ＞＞　必要なものは特にないありがとうございますと言われました。最近色々な問題で苦労をしているそうです。
///   ＞＞　お体調にいつもお気をつけてくださいと挨拶がありました。
/// 
/// - YeSongママに次の内容を伝えてください。
/// 1. ベランダのドアの規格が理解できません。ベランダが3つだと知っていますが、ベランダのドアはなぜ6つを要求するのですか？
/// 各門でウィンドウの規格を教えてください
///   ＞＞　ベランダのドアは３つでした。（やり戸式ですので、やり戸門は６つです）材質はジュラルミン。
///   ＞＞　30は太さで、各門の中身は２重の桶グラスが良さそうです。後で曇っているビニルを貼ることも可能ですから。
/// 2. ガス排風機は後で送りします。
///   ＞＞　分かりました。
/// 3. 現在、家にあるベッドの規格を教えてください。そのベッドを使おうと思います。
///   ＞＞　200 * 135cm  （２人用ベッドの標準）
///   
/// 以下の内容を早めに回答してくださいと話がありました。
/// ・行事が良く行われる地域ですので、工事を早めに終えないと行けない。
///   ちなみに、工事は連続的に行わないと色々良くない。（工事の質、作業者の問題）
///   現在１段階の工事は終り、タイルや壁紙を貼る作業が２－３日内に開始できる状態です。
/// ・タイル、人造大理石板、壁紙
///   →　国内購入して工事を進めるか　　費用は少しもっと掛かるかも知れませんが、調和や期間短縮の側面で良い
///   →　中国で購入して工事は来年に続ける　　調和が合うものを購入できるか心配、工事は暫定中止して来年に再開する
///   
///  追加内容がありました。
///  ・ベランダ門を一般門＋断熱窓の形式にする場合、門の幅は85cmにします。
///    やり戸式だと風が入る可能性もありますので、固定部分のある一般門（材質は同じジュラルミン）にする場合、門の幅は85cmになります。
///  ・家が小さいですので、タイルサイズも小さいものが良いです。
///   　床　800 * 800 ⇒ 600 * 600
///   　壁　400 * 800 ⇒ 300 * 600（装飾タイル） 
///   
