...
/// equivocate.
/// 
/// 回答の内容を確認しました。ありがとうございます。
/// - YeSongママに次の内容を伝えてください。
/// * 9月2日月曜日に資金1700$が到着する予定です。YeSongママの名前で電話が来るはずです。
/// 受付したらお知らせください。
/// * 今の家に工事を始めてください。
/// YeSongパパが話したように倉庫を壊して洗面所と連結してください。
/// 洗面所に洗濯機を入れてシャワジャンも設置しようと思います。シャワギや水道串などいろんな品物は9月に送っています。
/// * 9月に用品を送るのにYeSongお婆さん家とYeSong家,お兄さんの家に送る品物を送っています。
/// 品物を受け取る人は YeSongママの名前にしましたので受け取ってください。
/// * 去年12月にお兄さんの家に送ったトランクがありました。（お兄さんの下着、母の下着、靴、薬などがありました。）
/// 私が知る限りではYeSong家に保管していると知っていますが兄さんに渡したか確認をお願いします。
/// * ドアサイズ90*200、90*210の2個がありますが、ここでどれが大きな部屋のドアなのか教えてください。
/// - rgh先生に次の内容を伝えてください。
/// お兄さんに電話して、お兄さんがほぼ正常な状態の条件で、お兄さんが受けると言ったら渡してください。
/// お兄さんがちょっと痛かったり、受け取らないと言ったら、私が回国した後に渡しても大丈夫です。
/// 
/// 上記の内容は早速伝えるようにします。
/// 先日YeSongママからの依頼内容がありましたので、お送り致します。
/// １．部屋のドアは 90 * 200 で統一しましょう。
/// ２．倉庫には水タンクを設置する必要がある。そうするとシャワーも使えるようになる。
/// ３．タイル、天井板は税関の手続きが複雑だそうなので、やめた方が良さそう。
/// ４．台所は頼んだ通り対応できる。
/// ５．台所ベランダ：保温工事する；　ガスコンロと排風機は台所ベランダに配置；
/// 　　ガスTongは小さい部屋のベランダに配置した方が良い。
/// 　　ガス水加熱機はトイレに設置する。
/// ６．電気工事は全部やり直す。
/// 　　壁紙：大きい部屋45m²、小さい部屋30m²、複道 26m²
/// ７．８．は既に伝えた内容です。（工事開始の確定、新家の購入可否）
/// 
/// 
/// YeSongパパに確認した所、1700ドルは受け取ったそうです。
/// 工事は2日後に開始するそうです。
/// 荷物は9月上旬中に届くので、連絡が来たら受け付けてくださいと話しておきました。
/// 昨年お兄さんに伝えたトランクはお兄さんが退院した後にすぐ伝えたそうです。
/// お母さんのものは万が一のため伝えませんでしたが、それをお兄さんの妻に弟からの挨拶として伝えることはどうか聞きました。
/// お兄さんは今年中には１ヶ月間隔で再発防止の治療を行い、来年からは３ヶ月ごとに治療を行うつもりだそうです。（今もその治療計画通り治療しているそうです）
/// だいぶ好転しましたので、心配しなくて良いそうです。
/// お兄さんは社会保障の手続きをやめて軽労働ができる職場に手続きしたいそうです。
/// そのため、その方向で進めようとしている中だそうです。
/// 
