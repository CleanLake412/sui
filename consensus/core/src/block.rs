...
/// 
/// 
/// 以前の内容を確認しました。本当にありがとうございます。
/// - HSMに連絡する時、次の内容も伝えてください。
/// 荷物の運搬費は、私が後で祖国に帰った後や、連絡が出たら決算することにしてください。
/// 全体的には運搬費用は4500CNY程度がかかりました。丹東-平壌運送費がエアコン1個当たり900CNY、浄水器は1個当たり400CNYで費用が少しかかりました。
/// お兄さんが今まで過ぎ去った時期の記憶を忘れてしまうことがあるので、運搬費用はお兄さんに払わないでください。
/// 後に私と直接決算することで予想してください。
/// - ここではタイルやドアーなどの資材を中国で購入してしようと思います。
/// ですから、基本的な美装や便器、洗面台の設置などだけして工事を中止してください。
/// 資材を購入した後、来年に出て続けようと思います。
/// 9月28日に入る人便があってそちらに手紙を送りましたので、届いたら確認してください。
/// ベランダに出る窓枠を作業したら、スペースの長さと高さを教えてください。 
///
/// （確認点）ベランダに出る窓枠を作業したら、スペースの長さと高さを教えてください。 
/// ・引き戸式、一般門＋固定部分（断熱式）か明確にする方が良いと思います。
/// ・スペースは先日伝えたベランダ門サイズのママです。それが窓枠を設置するスペースになります。
///  >>> ここでは引き戸式にしたいのですが、そちらでは風が入ることができると言って他の方式をお勧めしますが、一応は基本美装だけして来年に資材を全部持って入ってすることにしようと思います。
///  >>> ベランダの大きさと関連して、次のようにドアの大きさが来ましたが
///  >>> 「門サイズ　　ベランダ　149 * 199 * 30 cm　　６つ」
///  >>> 一つのベランダに2つのドアを設置するということですが、全体の大きさを知りたいという意味です。
///
///　■ 再確認した結果をお送り致します。(9/30)
///　ベランダ門枠のサイズは 149 * 199 cm で、太さが30 cm です。
///　引き戸式にする場合、上記の情報を制作側に伝えれば良いと思います。枠以外は全部グラスにした方が良いと言われていました。
///　断熱式にする場合、固定部分があって門は一般の門になります。固定部分も門も枠以外はグラスにした方が良いとのことです。（門の幅は80~85cmが理想）
///　（断熱式の様子は door.png としてアップしました）
///　
///　追加(10/02)
///　- To YeSongママ
///　9月28日に手紙を送ったんですが、届いたか確認してください。
///　送った洗面台と便器はそのまま設置してください。設置上の問題があれば教えてください。
///　ドアやタイルなどはすべて用意して送ろうと思うので、それは到着してから来年の初めにしようと思います。
///　＞＞　上記の内容は伝えましたが、再確認します。手紙が届いたか確認します。
///　
///　追加(10/03)
///　- To YeSongママ
///　1. トイレの床を破って、居室の床の高さに似ているか教えてください。
///　2. トイレのドアの高さを200cmにしたらどうですか？
///　門の高さを200に統一すればいいと思いますが、可能かどうか確認をお願いします。
///　3. ベランダのドアは左右と上の部分をできるだけ破れましたか？
///　ドア枠の規格を見ると予想より小さいのですが、ドア枠の左右の壁は破れない壁ですか？
///
/// ----------------------------------10/5---------------------------------------------------
/// 手紙を読んでYeSongパパからの回答です。（Miryongママに回答するようです）
/// １．ミルク粉は３個しかなかった。
/// 　　ここで検索を受け取る際にはなくなったことはなかったが、
/// 　　荷物が運搬されてくる途中に食品などはなくなる場合があるそう。
/// ２．靴のサイズ
/// 　　Miryongママのお兄さん：　250  ←　送ってきた靴のサイズが265だったので、大きいすぎる
/// 　　Yesongパパ： 270
/// ３．ベランダ門
/// 　　写真を確認したが、そんな形式の門は冬に寒い。
/// 　　ホテルや大きい家に合うかも知れないが、今の家には合わないと思う。
/// 　　・やり戸門の場合、ベランダが３つでそれぞれやり戸門は２つずつ必要
/// 　　・断熱式の場合、固定部分＋一般門（両方ともグラス）　←　断熱式が現実的
/// 
/// 　　※右左を最大2mまで増やすことは可能だが、そうすると熱管理が難しいだよ。
/// 　　　どうしても2mまで増やしたいのであれば、それは可能だ。
/// 　　　そうすると、200 * 199 * 30 になる。（200: 右左の幅、199: 高さ、 30: 厚さ）
/// 
///   ---
/// 
///   部屋の門
/// 　　→　門を開く方向をちゃんと考えなければならない。
/// 　　→　外から部屋に入る場合、蝶番(접철)が右か左か；　門を押して開くか、引っ張って開くか
/// 
/// 　　・大きい部屋　　蝶番：右、押して開く
/// 　　・台所　　　　　　蝶番：右、押して開く
/// 　　・小さい部屋　　蝶番：左、引っ張って開く　　　←　家の構造を思い出してみなさい
/// 　　・トイレ　　　　　蝶番：左、引っ張って開く
/// 　　・出入門　　　　蝶番：右、引っ張って開く（外側）　　←　外から家に入りながら
/// 　　　　　　　　　　　　　　　　　　押して開く（内側）
/// ４．台所
/// 　　準備作業は終わった。帰国して完成したら良い。
/// 　　ベランダが南向だから、冷蔵庫はベランダに置いてはいけない。
/// ５．便器と洗面台はタイルを貼ってから設置できる。
/// ６．タイル価格
/// 　　800 * 800  床タイル　8ドル
/// 　　400 * 800　　壁タイル 2.5ドル
/// 　　タイルは税関で関税として17%を支払うそう。
/// 　　送達費用もあるし、ここで受け付けて家まで運搬して来るのも50ドルぐらいは掛かる。（車、人力）
/// 
/// 　　家に暖房が回るから冬にもタイル作業は可能だと思う。
/// ７．ベッドサイズ：　200 * 135
/// ８．エグ攪拌機：　購入依頼を送った直後にRGH先生からお兄さんへ荷物を伝えたそう。
/// 　　その中にエグ攪拌機があったそう。
/// ９．10/6日頃に一次作業が終わる。（費用は先日連絡した通り）
/// 　　Miryongママが話したように工事は中止する。
/// 
/// 　　７～８つの単位や関係者が監督しているから、それのためも費用も掛かったが、再開する際にまた掛かる可能性がある。
/// 　　色々中止してから再開するのは手間が掛かりそう。
/// １０．サンダー：　棒式で頼む　（タイルなどを切る際に使う工具）
/// 　　打撃ドリル：　コンクリートに穴を開けたりする際に使う工具；　個人用だから、小さいトランクサイズ
/// １１．女性用の自転車
/// 　　車輪サイズ：　14インチ（桶ダイヤ）
/// 　　自転車のサドル(鞍裝)に緩衝装置が付いているもの
/// 
/// ＊私に対してロシア派遣の幹部事業が始まっている。
/// 　いつ出発するかは未定ですが、もし私が席を外した場合にも工事計画は作成して置いたので、別の方でも引き続けることは可能。
/// 
/// ＊門サイズ（高さ）を200cmに統一することは難しい。
/// 　組立式だから上のコンクリート部分は勝手に手を入れることができない。
/// 
/// ＊冷蔵庫、エアコンは関税があるので、送る際に注意が必要。（それぞれ50ドルぐらい）
/// 
