...
/// まずは工事を始めてください。
/// 努力費800、資材費900に対する費用1700$は早いうちに送るようにしますので、始めてください。
/// 家に器やいろいろな物、服がありますが、問題がないようによく見てください。
/// * 工事をすれば作業はいつまでかかると見ていますか？
/// * 平壌駅近くの中区域の3間の100~120平方メートルの住宅価格（ある程度は装飾した家）を確認して教えてください。
/// 
/// →　工事は開始してくださいと伝えました。
/// 　１ヶ月ぐらい掛かるそうです。
/// 　平壌駅近くの中区域の3間の100~120平方メートルの住宅価格：家だけで6~7万ドル、家の手入れには1~2万掛かるそうです。
/// 　　　　　合計7~9万ドルぐらいは掛かるそうですが、工事はそのまま開始して良いか再確認の依頼がありました。
/// 　　　　　工事するとしても今の家の価値がそのぐらい上がる担保はないそうです。
/// 　お兄さんは心配しなくて良いそうです。HH先生が帰ると元気で仲良く家庭生活をしているお兄さんを会えるそうです。
/// 　お兄さんの妻にありがたい気持ちを伝えてほしいと話がありました。（お兄さんの近くで妻が一番苦労をしているそうです）
/// 
/// - rgh先生に次の内容を伝えてください。
/// 椅子を送ります。9月2日にDanDongで発送されますが、いつPYに到着するかはわかりません。
/// 箱番号60番が椅子です。rchiで名前で発送しますので荷物が届いたらrchiで連絡がいくと思います。
/// 価格は500CNYです。発送費はまだ確定していないので、後でお知らせします。
/// 
/// 　→　伝えました。
/// 　　RGH先生からお兄さんが治療を受けている間に自分が預かっていた荷物（１ボックス）を今は渡して良いか質問がありました。
/// 
/// - hsm先生に次の内容を伝えてください。
/// 私に預けたすべての荷物を9月2日にDanDongで発送されますが、いつPYに到着するかはわかりません。
/// 元々は冷温風機が1台できましたが、2台とも含めて全部送っています。
/// すべての荷物を受け取る人は、ここにいて召喚されて入ったリチュンイル（191-622-3855、195-704-9598）ですが、到着したら連絡をするようにします。
/// 現在荷物を送るのがますます難しくなっています。
/// 
/// 　→　伝えました。
/// 　　RCHIは今は（191-622-3855、195-704-9598）の番号は使っていないそうです。
/// 　　ただ親戚が使っていますので、荷物が届く時点で電話が来たら自分に教えて下さいと連絡しておくそうです。
